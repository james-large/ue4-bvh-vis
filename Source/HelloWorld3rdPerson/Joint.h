...
class Joint {
 public:
  ...
  float channel_data(unsigned frame, Channel channel) const {
    return channel_data_[frame][channel_indices_.at(channel)];
  }


  /** Gets the local transform for this joint for all frames
   *  @return  The joint's local transform
   */
  std::vector <FTransform> ltm() const {
    return ltm_;
  }

  /** Gets the local transform for this joint for selected frame
   *  @param   frame    The frame for which ltm will be returned
   *  @return  The joint's local transform for selected frame
   */
  FTransform ltm(unsigned frame) const {
    return ltm_[frame];
  }
   ...
   */
  FVector pos(unsigned frame) const {
    return pos_[frame];
  }
  ...
  /** Sets local transform for selected frame
   *  @param  transform  The local transform to be set
   *  @param  frame   The number of frame for which you want set ltm. As
   *                  default it is set to 0.
   */
  void set_ltm(const FTransform transform, unsigned frame = 0) {
    if (frame > 0 && frame < ltm_.size())
      ltm_[frame] = transform;
    else
      ltm_.push_back(transform);
  }
   ...
   *                  default it is set to 0.
   */
  void set_pos(const FVector pos, unsigned frame = 0) {
 ...
 private:
  ...
  std::map <Channel, int> channel_indices_;
...
//   /** Local transformation matrix for each frame */
  std::vector <FTransform> ltm_;
//   /** Vector x, y, z of joint position for each frame */
  std::vector <FVector> pos_;
};
